...
void main() {
  group('설정 완료(baseUrl 주입): URL 조립 + 열기', () {
    ...
    test('약관/지원/본인인증 각 경로로 조립', () async {
      final WebBridge b = make();
      await b.openTerms();
      await b.openSupport();
      await b.openIdentityVerify();
      expect(opened.map((Uri e) => e.path).toList(), <String>[
        WebBridgeConfig.termsPath,
        WebBridgeConfig.supportPath,
        WebBridgeConfig.identityVerifyPath,
      ]);
    });
  ...
  group('미확정(baseUrl 비었음): 열지 않고 안내 폴백', () {
    test('launcher 미호출 + notConfigured + buildUri null', () async {
      ...
      expect(await b.openTerms(), WebOpenResult.notConfigured);
      expect(await b.openSupport(), WebOpenResult.notConfigured);
      expect(called, false); // 가짜 URL 을 만들지도, 열지도 않는다.
      expect(b.buildUri(WebBridgeConfig.termsPath), isNull);
    });
    ...
    test('기본 WebBridge() 는 WebBridgeConfig 설정값을 그대로 반영', () {
      ...
      final Uri? uri = WebBridge().buildUri(WebBridgeConfig.termsPath);
      expect(uri, isNotNull);
      expect(uri!.origin, WebBridgeConfig.baseUrl);
      expect(uri.path, WebBridgeConfig.termsPath);
    });
  });
}
