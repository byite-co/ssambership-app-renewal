...
/// 빌드 대상 고정 — vc22 후보(1.0.0+22 범프 커밋) head. 전체 40자 SHA 여야 한다.
const String kSourceSha = '20ff0b8f1293b8aa0cf14aa0e6ac623ed1442069';
...
void main() {
  ...
  group('트리거 계약 — workflow_dispatch 단독', () {
    ...
    test('수동 확인 입력 2종 — required + default false + 사람 확인 기록 취지', () {
      ...
      for (final String name in <String>[
        'confirm_version_code_22_unused',
    ...
    test('job 은 두 확인 입력이 모두 true 일 때만 시작한다', () {
      ...
      expect(cond, contains('confirm_version_code_22_unused == true'));
  ...
  group('권한·환경·동시성 계약', () {
    ...
    test('concurrency 고정 그룹 + cancel-in-progress=false', () {
      ...
      expect(conc['group'], 'android-signed-release-candidate-22');
  ...
  group('빌드 대상 고정 계약', () {
    ...
    test('버전·패키지·URL·테스트수 기대값 고정', () {
      expect(env['EXPECTED_PR'].toString(), '54');
      expect(env['EXPECTED_VERSION'].toString(), '1.0.0+22');
      expect(env['EXPECTED_VERSION_NAME'].toString(), '1.0.0');
      expect(env['EXPECTED_VERSION_CODE'].toString(), '22');
      ...
      expect(env['EXPECTED_TEST_COUNT'].toString(), '1510');
