...
- **작성일:** 2026-07-29 (rev 5 — 웹 정본 지시서 rev 5와 동기: 커뮤니티 작성 멘토 전용(`ROLE_NOT_MENTOR`) 정책 반영, room 참조 규칙 컬럼별 표 동결, F12 오류코드 우선순위·anomaly 계약, M2 retired 번호 정책. rev 4: topup 정본 `idempotency_key` 단독(`ref_text` 제거)·6필드 단일 계약. rev 3: `HD-1` 고정·보상 삭제 RPC 대조표 연결. rev 2: 공용 함수 대조표 의무·hard DELETE 분리. 상세는 웹 정본 지시서 A-5·A-6·A-10 참조)
...
9. **커뮤니티 작성 멘토 전용(rev 5, 웹 정본 A-10):** 확정 제품 정책은 "멘토만 작성, 학생·비로그인·관리자 작성 CTA 제거". 앱 계약 v1.0의 `ROLE_NOT_ALLOWED`(학생·멘토 모두 허용) 정의를 폐기하고, F4 create는 `users.role='mentor'`만 허용 + 오류코드 **`ROLE_NOT_MENTOR`**로 수정한다. 앱 UI의 학생 작성 CTA 제거를 앱 측 의무로 기재. 잔존 선택 2건(승인 멘토 한정 여부, 기존 학생 작성글 처리 범위)은 v1.1 작성 세션이 동결한다.
