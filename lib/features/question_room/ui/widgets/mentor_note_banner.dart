...
/// 답변 화면 상단 노트 배너(A-5 §2-1) — 1줄로 접혀 있고 탭하면 펼쳐진다.
/// 답변을 쓰는 동안 이 학생의 맥락(내가 마지막으로 남긴 노트)이 눈앞에 있게 한다.
class MentorNoteBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final MentorNoteParts? parts = n == null ? null : MentorNoteParts.parse(n.body);
              ...
              if (expanded) ...<Widget>[
                ...
                if (parts != null && parts.isStructured) ...<Widget>[
                  if (parts.weakness != null)
                    _PartRow(label: kMentorNoteWeaknessLabel, text: parts.weakness!),
                  if (parts.next != null)
                ...
                Align(
                  ...
                  child: TextButton(
                    onPressed: onOpenAll,
                    child: Text(noteCount > 0 ? '노트 $noteCount개 전체 보기' : '연결노트 열기'),
                  ),
                ),
              ],
            ],
