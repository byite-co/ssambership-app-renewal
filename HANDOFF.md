...
- **Android**: 릴리스 빌드·서명 키·Play Store 등록. **iOS**: 네이티브 설정 정합 완료(조회 스킴·수출규정 키·프라이버시 매니페스트 — 계약 가드 `test/contracts/ios_release_config_contract_test.dart`). 남은 것 = macOS 실빌드 검증 + 오너 결정(번들 ID `com.ssambership.app` 유지 vs `.edu` 정렬 — **첫 업로드 후 변경 불가**) + Apple Developer 가입·서명·App Store Connect. **절차·심사 리스크 정본: `docs/IOS_RELEASE_RUNBOOK.md`** (⚠️ iOS 스토어 빌드에선 `SUBS_MANAGE_LINK_ENABLED` 주입 금지 — Apple 3.1.1).
